...
All 121 users registered before the fixed cutoff date `2026-03-08T00:00:00.000Z` receive permanent free access. The `users.isLegacyFree` boolean field marks them; their `legacy_free` subscriptions have `endDate = 2099-12-31` (effectively never expire). The `/api/subscription-status` endpoint checks `isLegacyFree` before the normal subscription check and returns `{ hasActiveSubscription: true, isLegacyFree: true, plan: "legacy_free" }`. An auto-backfill runs at every server startup (`runLegacyBackfill()` in routes.ts) and is idempotent. A dismissible amber banner is shown to legacy users on the exercises page. Admin can view stats and re-run backfill at `/admin` → "الوصول المجاني القديم" tab. All new UI strings are translated in all 13 languages.
